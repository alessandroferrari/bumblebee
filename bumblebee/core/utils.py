...
def export_to_onnx(model, context_length, vocab_size, output_path, device):
    sample_input = torch.randint(0, vocab_size, (1, context_length), dtype=torch.int64, device=device)
    onnx_model = torch.onnx.export(model, sample_input,output_path, dynamo=False)