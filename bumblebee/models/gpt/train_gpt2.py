...
if __name__ == "__main__":
    ...
    setings, params = download_and_load_gpt2(gpt_model_configs["gpt2-small"]["size"], "models_zoo/")
    ...
    PRETRAINED_GPT_MODEL_CONFIG.update(gpt_model_configs["gpt2-small"])

    train_dataset = create_dataloader(txt=train_txt, tokenizer=tokenizer,
                                      batch_size=1, max_length=PRETRAINED_GPT_MODEL_CONFIG["context_length"],
                                      stride=PRETRAINED_GPT_MODEL_CONFIG["context_length"], shuffle=True, drop_last=True)
    eval_dataset = create_dataloader(txt=eval_txt, tokenizer=tokenizer,
                                     batch_size=1, max_length=PRETRAINED_GPT_MODEL_CONFIG["context_length"],
                                     stride=PRETRAINED_GPT_MODEL_CONFIG["context_length"], shuffle=False, drop_last=False)
