...
class DummyGPTModel(nn.Module):
    def __init__(self, vocab_size, num_embeddings=100, context_length=10, dropout_rate=0.1, num_layers=12, num_heads=4):
        ...
        self.transformer_blocks = nn.Sequential(*[DummyTransformerBlock(num_embeddings, num_heads, dropout_rate) for _ in range(num_layers)])
    ...
    def forward(self, in_idx):
        ...
        pos_embds = self.position_embedding(torch.arange(context_legth, device=in_idx.device))
...
if __name__=="__main__":
    ...
    dummy_gpt = DummyGPTModel(vocab_size=50257, num_embeddings=5, context_length=4, dropout_rate=0.25, num_layers=6, num_heads=4)
